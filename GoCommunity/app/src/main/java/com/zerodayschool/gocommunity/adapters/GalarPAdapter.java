...
public class GalarPAdapter<galars> extends RecyclerView.Adapter<GalarPAdapter.ViewHolder>{



    // Define listener member variable
    private GalarPAdapter.OnItemClickListener listener;
    ...
    public void setOnItemClickListener(GalarPAdapter.OnItemClickListener listener) {
    ...
    private List<Galar> originalItems;//buscador
    public GalarPAdapter(List<Galar> galars){this.galarList = galars;
        this.originalItems = new ArrayList<>();//buscador
        originalItems.addAll(galars);//buscador
    }



    public GalarPAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        Context context = parent.getContext();
        LayoutInflater inflater = LayoutInflater.from(context);

        View galarView = inflater.inflate(R.layout.card_preview_galar, parent, false);
        GalarPAdapter.ViewHolder viewHolder = new GalarPAdapter.ViewHolder(galarView);

        return viewHolder;
    }


    public void onBindViewHolder(GalarPAdapter.ViewHolder holder, int position) {
    ...
    public void filter (String strSearch){
        ...
        if (longuitud == 0){
            galarList.clear();
            galarList.addAll(originalItems);
        }else {

            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N){
                List<Galar> collecion = galarList.stream().filter(i -> i.getNombre().toLowerCase().contains(strSearch.toLowerCase()))
                        .collect(Collectors.toList());
                galarList.clear();
                galarList.addAll(collecion);
            }else {
                for (Galar c: originalItems){
